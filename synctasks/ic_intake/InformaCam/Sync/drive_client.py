...
from conf import DEBUG, MONITOR_ROOT, GOOGLE_DRIVE_CONF

class DriveClient(InformaCamSyncClient):
	def __init__(self, mode):
		InformaCamSyncClient.__init__(self, GOOGLE_DRIVE_CONF['log'], mode=mode)
		...
		try:
			with open(GOOGLE_DRIVE_CONF['p12'], 'rb') as key:
				with open(GOOGLE_DRIVE_CONF['client_secrets'], 'rb') as secrets:
					...
					files = self.service.children()
						.list(folderId=GOOGLE_DRIVE_CONF['asset_root']).execute()
					self.files_manifest = []
	...
	def listAssets(self, omit_absorbed=False):
		...
		for f in files['items']:
			...
			try:
				clone = self.service.files()
					.copy(fileId=f['id'], body={'title':f['id']}).execute()
	...
	def isAbsorbed(self, file_name, mime_type):
		if self.mode == "sources":
			if mime_type != mime_types['zip']: return True
		elif self.mode == "submissions":
			if mime_type == mime_type['zip']: return True
